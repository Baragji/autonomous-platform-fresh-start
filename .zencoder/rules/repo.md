...
- `.automation/evidence/`: machine-generated evidence only (coverage.json, healthz_sweep.json, e2e_request_response.json, prod_env_guard.json, v5-report.json, ATTACHMENT_MANIFEST.json, TAMPERING_ALERT.txt, .gitkeep). SBOMs must NOT be placed here.
- `.automation/supply_chain/`: stores SBOM at `sbom.json` and related supply-chain artifacts.
...
- **Objective summary**: Deliver Option A pipeline—Smart MCA → Smart Specialists → Zero-Trust Validator—with deterministic validator, bounded remediation loop (≤3 attempts), CI-enforced evidence, Cosign keyless attestation (release only), OpenSSF Scorecard (record score on hardening; enforce minimum on release), and protected `release/secure-<UTC>` branch.
...
- SBOM is written to `.automation/supply_chain/sbom.json` (outside the evidence allowlist). Cosign predicate uses this path.
