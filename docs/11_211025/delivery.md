...
services:
  ...
  minio-init:
    ...
    entrypoint: >
      /bin/sh -c "
      mc alias set local http://minio:9000 ${MINIO_ROOT_USER:-admin} ${MINIO_ROOT_PASSWORD:-change-me-now};
      mc mb --ignore-existing local/${EVIDENCE_BUCKET:-evidence};
      exit 0;
      "
  ...
  postgres:
    image: postgres:16
    environment:
      POSTGRES_PASSWORD: ${POSTGRES_PASSWORD:-change-me-now}
  ...
  # Application services (uncomment after creating Dockerfiles in Week 1)
  # gateway:
  #   build: ./apps/gateway
  #   ports:
  #     - "8080:8080"
...
OTEL_SERVICE_NAME=autonomous-platform
```
...
> `packages/contracts/openapi/planner-ra.openapi.yaml`

```yaml
openapi: 3.1.0
info:
  title: Planner (RA) Service API
  version: 1.0.0
servers:
...
> `packages/contracts/openapi/architect-aa.openapi.yaml`

```yaml
openapi: 3.1.0
info:
  title: Architect (AA) Service API
  version: 1.0.0
servers:
...
> `packages/contracts/openapi/implementer-ia.openapi.yaml`

```yaml
openapi: 3.1.0
info:
  title: Implementer (IA) Service API
  version: 1.0.0
servers:
...
> `packages/contracts/openapi/runner-da.openapi.yaml`

```yaml
openapi: 3.1.0
info:
  title: Runner (DA) Service API
  version: 1.0.0
servers:
...
> `packages/contracts/openapi/security-sa.openapi.yaml`

```yaml
openapi: 3.1.0
info:
  title: Security (SA) Service API
  version: 1.0.0
servers:
...
> `packages/contracts/openapi/quality-qa.openapi.yaml`

```yaml
openapi: 3.1.0
info:
  title: Quality (QA) Service API
  version: 1.0.0
servers:
...
> `packages/contracts/openapi/finops-fops.openapi.yaml`

```yaml
openapi: 3.1.0
info:
  title: FinOps (FOPS) Service API
  version: 1.0.0
servers:
...
> `packages/contracts/openapi/db-layer-dba.openapi.yaml`

```yaml
openapi: 3.1.0
info:
  title: DB Layer (DBA) Service API
  version: 1.0.0
servers:
...
export type WorkKind = "plan"|"design"|"implement"|"run"|"scan"|"qualify"|"cost-check"|"db-op";
...
export class WorkBus {
  ...
  async request<T = any>(kind: WorkKind, payload: object, timeoutMs = 60000): Promise<T> {
    if (!this.nc) throw new Error("NATS not initialized");
    const subj = `work.${kind}.request`;
    const msg = await this.nc.request(subj, sc.encode(JSON.stringify(payload)), { timeout: timeoutMs });
...
```ts
import { workflow, bus } from "./graph/state";
...
const minio = new Client({
  ...
  secretKey: process.env.MINIO_ROOT_PASSWORD || "change-me-now"
});
...
async function main() {
  ...
  for await (const m of sub) {
    ...
    child.on("exit", async () => {
      ...
      // Reply using NATS request/reply pattern
...
async function main() {
  ...
  app.get("/healthz", (_req, res) => res.sendStatus(204));
  app.get("/readyz", (_req, res) => res.sendStatus(204));
  app.listen(7011, () => console.log("planner-ra listening on :7011"));

  const sub = nc.subscribe("work.plan.request");
  console.log("planner-ra listening on work.plan.request");

  for await (const m of sub) {
    const msg = JSON.parse(sc.decode(m.data));
    const { execId, userIntent } = msg;

    // TODO: Call LLM to decompose userIntent into tasks
    const res = {
      tasks: [
...
> `services/architect-aa/src/server.ts`
...
async function main() {
  ...
  app.get("/healthz", (_req, res) => res.sendStatus(204));
  app.get("/readyz", (_req, res) => res.sendStatus(204));
  app.listen(7012, () => console.log("architect-aa listening on :7012"));

  const sub = nc.subscribe("work.design.request");
  console.log("architect-aa listening on work.design.request");

  for await (const m of sub) {
    const msg = JSON.parse(sc.decode(m.data));
    const { execId } = msg;

    // TODO: Generate ADR and OpenAPI contract
    const res = {
      adrUrl: `http://localhost:9000/evidence/${execId}/adr.md`,
      contractUrl: `http://localhost:9000/evidence/${execId}/openapi.yaml`
    };
    m.respond(sc.encode(JSON.stringify(res)));
  }
}
...
> `services/implementer-ia/src/server.ts`
...
async function main() {
  ...
  app.get("/healthz", (_req, res) => res.sendStatus(204));
  app.get("/readyz", (_req, res) => res.sendStatus(204));
  app.listen(7013, () => console.log("implementer-ia listening on :7013"));

  const sub = nc.subscribe("work.implement.request");
  console.log("implementer-ia listening on work.implement.request");

  for await (const m of sub) {
    const msg = JSON.parse(sc.decode(m.data));
    const { execId } = msg;

    // TODO: Generate code patch using LLM
    const res = {
      patchUrl: `http://localhost:9000/evidence/${execId}/code.patch`
    };
    m.respond(sc.encode(JSON.stringify(res)));
  }
}
...
> `services/security-sa/src/server.ts`
...
async function main() {
  ...
  app.get("/healthz", (_req, res) => res.sendStatus(204));
  app.get("/readyz", (_req, res) => res.sendStatus(204));
  app.listen(7015, () => console.log("security-sa listening on :7015"));

  const sub = nc.subscribe("work.scan.request");
  console.log("security-sa listening on work.scan.request");

  for await (const m of sub) {
    const msg = JSON.parse(sc.decode(m.data));
    const { execId } = msg;

    // TODO: Run Semgrep, Trivy, generate SBOM
    const res = {
      sarifUrl: `http://localhost:9000/evidence/${execId}/semgrep.sarif`,
      sbomUrl: `http://localhost:9000/evidence/${execId}/sbom.json`,
...
> `services/quality-qa/src/server.ts`
...
async function main() {
  ...
  app.get("/healthz", (_req, res) => res.sendStatus(204));
  app.get("/readyz", (_req, res) => res.sendStatus(204));
  app.listen(7016, () => console.log("quality-qa listening on :7016"));

  const sub = nc.subscribe("work.qualify.request");
  console.log("quality-qa listening on work.qualify.request");

  for await (const m of sub) {
    const msg = JSON.parse(sc.decode(m.data));
    const { execId } = msg;

    // TODO: Analyze coverage from runner-da output
    const res = {
      lineCoveragePct: 85,
...
> `services/finops-fops/src/server.ts`
...
async function main() {
  ...
  app.get("/healthz", (_req, res) => res.sendStatus(204));
  app.get("/readyz", (_req, res) => res.sendStatus(204));
  app.listen(7017, () => console.log("finops-fops listening on :7017"));

  const sub = nc.subscribe("work.cost-check.request");
  console.log("finops-fops listening on work.cost-check.request");

  for await (const m of sub) {
    const msg = JSON.parse(sc.decode(m.data));
    const { execId, budgetLimit } = msg;

    // TODO: Calculate actual LLM token costs
    const res = {
      totalCost: 0.45,
      overBudget: budgetLimit ? 0.45 > budgetLimit : false
    };
    m.respond(sc.encode(JSON.stringify(res)));
  }
}
...
> `services/db-layer-dba/src/server.ts`
...
const pool = new Pool({
  ...
  password: process.env.POSTGRES_PASSWORD || "change-me-now",
...
async function main() {
  ...
  app.get("/healthz", (_req, res) => res.sendStatus(204));
  app.get("/readyz", (_req, res) => res.sendStatus(204));
  app.listen(7018, () => console.log("db-layer-dba listening on :7018"));

  const sub = nc.subscribe("work.db-op.request");
  console.log("db-layer-dba listening on work.db-op.request");

  for await (const m of sub) {
    const msg = JSON.parse(sc.decode(m.data));
    const { op, table, data } = msg;

    // TODO: Execute DB operations (insert, query, etc.)
    const res = { success: true, rowCount: 1 };
    m.respond(sc.encode(JSON.stringify(res)));
  }
}
