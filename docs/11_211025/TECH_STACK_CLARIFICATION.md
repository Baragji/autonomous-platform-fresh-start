...
### Week 1: TypeScript Skeleton (All Services)

**Decision:** ALL services start as TypeScript for Week 1 to achieve E2E execution quickly.

**Rationale:**
- Faster to get E2E working (single language, shared tooling)
- Easier to debug NATS request/reply integration
- Meet Week 1 exit criteria (all services running, gates passing)

**Skeleton services (canned responses):**
- planner-ra: TypeScript stub (returns hardcoded tasks)
- architect-aa: TypeScript stub (returns mock ADR/OpenAPI URLs)
- implementer-ia: TypeScript stub (returns mock patch URL)
- runner-da: TypeScript stub (runs npm test, uploads JUnit/coverage)
- security-sa: TypeScript stub (returns empty SARIF, 0 findings)
- quality-qa: TypeScript stub (returns 85% coverage)
- finops-fops: TypeScript stub (returns $0.45 cost)
- db-layer-dba: TypeScript stub (mock DB ops)

### Week 2-3: Replace with Language-Specific Implementations

**After Week 1 exit criteria met, replace stubs with:**

```bash
# Python services (LLM-heavy)
services/planner-ra/       # Python + FastAPI + LangChain
services/architect-aa/     # Python + FastAPI + LangChain
services/implementer-ia/   # Python + FastAPI + LangChain

# Go service (performance-critical)
services/runner-da/        # Go + Fiber (sandbox isolation)

# Rust service (security-critical)
services/security-sa/      # Rust + Axum (memory safety)
```

**Strategy:**
1. Keep TypeScript stub running (fallback)
2. Build Python/Go/Rust version alongside
3. Switch NATS subject when new version passes tests
4. Remove TypeScript stub after 1 week of stable operation

**This follows Article II (Anti-Refactoring):**
- ✅ NOT refactoring (adding parallel implementations)
- ✅ NOT breaking existing system (both run simultaneously)
- ✅ NOT "we'll rewrite later" (intentional 2-week plan)
